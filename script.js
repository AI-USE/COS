...
function startTest(mode) {
    ...
    elements.testProgressDisplay.textContent = `${state.testCurrentIndex + 1} / ${state.testWords.length}`;
...
function endTest() {
    ...
    elements.resultTotalQuestions.textContent = totalQuestions;
    elements.resultAccuracy.textContent = `${accuracy.toFixed(2)}%`;
    ...
    elements.testProgressDisplay.textContent = ''; // Clear progress display
    showScreen(elements.resultScreen);
}
...
function renderTestMeaningTab() {
    const previouslySelected = elements.testCourseSelectMeaning.value;
    populateCourseSelect(elements.testCourseSelectMeaning);
    // If a valid course was already selected in this dropdown, keep it.
    // Otherwise, sync it with the main screen's course.
    if (allWordsData[previouslySelected]) {
        elements.testCourseSelectMeaning.value = previouslySelected;
    } else {
        elements.testCourseSelectMeaning.value = state.currentCourse;
    }
    ...
    wordsInCourse.forEach(word => {
        ...
        labels.forEach(label => {
            ...
            if (stats.meaningChoiceTestCorrect === 0 && stats.meaningChoiceTestIncorrect === 0) {
...
function renderTestSpellingTab() {
    const previouslySelected = elements.testCourseSelectSpelling.value;
    populateCourseSelect(elements.testCourseSelectSpelling);
    // If a valid course was already selected in this dropdown, keep it.
    // Otherwise, sync it with the main screen's course.
    if (allWordsData[previouslySelected]) {
        elements.testCourseSelectSpelling.value = previouslySelected;
    } else {
        elements.testCourseSelectSpelling.value = state.currentCourse;
    }
    ...
    wordsInCourse.forEach(word => {
        ...
        labels.forEach(label => {
            ...
            labelStats[label].total++; // Count total words with this label in the course
            if (stats.spellingTestCorrect === 0 && stats.spellingTestIncorrect === 0) {
                labelStats[label].unseen++;
            }
        });
    });
...
function renderRecordSearchTab() {
    const previouslySelected = elements.searchCourseSelect.value;
    populateCourseSelect(elements.searchCourseSelect);
    // If a valid course was already selected in this dropdown, keep it.
    // Otherwise, sync it with the main screen's course.
    if (allWordsData[previouslySelected]) {
        elements.searchCourseSelect.value = previouslySelected;
    } else {
        elements.searchCourseSelect.value = state.currentCourse;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    elements = {
        ...
        testProgressDisplay: document.getElementById('test-progress-display'),
    ...
    // Event listeners for grade management course selection
    elements.testCourseSelectMeaning.addEventListener('change', renderTestMeaningTab);
    elements.testCourseSelectSpelling.addEventListener('change', renderTestSpellingTab);
