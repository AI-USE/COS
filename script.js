...
const elements = {
    ...
    customFilterPanel: document.getElementById('search-custom-filter-panel'),
...
function checkSpellingTestAnswer(typedText, targetText) {
    const normalizedTyped = normalizeTypingString(typedText).toLowerCase();
    const normalizedTarget = normalizeTypingString(targetText).toLowerCase();

    // まず、正規化された文字列が完全に一致するかチェック
    if (normalizedTyped === normalizedTarget) {
        return true;
    }

    // 次に、入力された文字がターゲットの先頭から一致し、残りが自動スキップ可能かチェック
    // 例: targetText = "hello〜world", typedText = "hello"
    // この場合、"〜world" が自動スキップ可能であれば true
    if (normalizedTyped.length <= normalizedTarget.length) {
        ...
        if (matchUntil) {
            // 入力された部分が一致している場合、残りのターゲット文字がすべてスキップ可能か確認
            let remainingTarget = normalizedTarget.substring(normalizedTyped.length);
            if (remainingTarget.length === 0) {
                return true; // 全て入力済み
            }
            
            // 残りの文字が全てスキップ対象（チルダ、スペース）であるかチェック
            for (let i = 0; i < remainingTarget.length; i++) {
                const char = remainingTarget[i];
                if (!(char === '~' || char === ' ' || char === '　')) { // normalizeTypingStringで~に変換されているため
                    return false; // スキップできない文字が含まれている
                }
            }
            return true; // 残りの文字は全てスキップ可能
        }
    }

    return false; // それ以外は不正解
}
...
function playWordAudio(word) {
    ...
    if (word.audio) {
        ...
        if ('speechSynthesis' in window) {
            ...
            createBeep(660, 0.1, 0.3); // Fallback to beep if no audio and no speech synthesis
        }
    }
}
...
elements.testTypingInput.addEventListener('input', (event) => {
    ...
    const targetText = state.currentWord.word;
    let newInputValue = elements.testTypingInput.value; // Use let for newInputValue here

    // DeleteキーとBackSpaceキーの処理を無効化
    if (event.inputType === 'deleteContentBackward' || event.inputType === 'deleteContentForward') {
        elements.testTypingInput.value = state.currentInput; // 変更を元に戻す
        return;
    }

    // 入力された文字が1文字増えた場合のみ処理
    if (newInputValue.length > state.currentInput.length) {
        const newChar = newInputValue[newInputValue.length - 1];
        let expectedChar = targetText[state.currentInput.length];

        // スキップ対象文字の自動処理
        // 全角チルダ、半角チルダ、全角スペース、半角スペースをスキップ対象とする
        const skippableChars = ['〜', '~', ' ', '　'];

        // 現在の入力位置がターゲット文字列の範囲内であるか確認
        if (state.currentInput.length < targetText.length) {
            // 期待される文字がス���ップ可能文字の場合、自動的にスキップ
            while (state.currentInput.length < targetText.length && skippableChars.includes(targetText[state.currentInput.length])) {
                state.currentInput += targetText[state.currentInput.length];
                elements.typingInput.value = state.currentInput; // 入力フィールドも更新
                playSound('key'); // スキップ時もキー音を鳴らす
                state.totalTypedChars++;
                state.correctTypedChars++;
                state.combo++;
                if (state.combo > state.maxCombo) state.maxCombo = state.combo;
                ...
                // スキップ後、次の期待文字を更新
                expectedChar = targetText[state.currentInput.length];
            }
        }

        // スキップ処理後に、まだ入力すべき文字が残っているか確認
        if (state.currentInput.length < targetText.length) {
            // 通常の文字入力判定
            if (isCharMatch(newChar, expectedChar)) {
                state.currentInput = newInputValue; // 正しい場合のみ更新
                state.totalTypedChars++;
                state.correctTypedChars++;
                state.combo++;
                if (state.combo > state.maxCombo) state.maxCombo = state.combo;
                ...
                state.missCount++;
                state.combo = 0;
                // 不正解の場合、入力フィールドを直前の正しい状態に戻す
                elements.typingInput.value = state.currentInput;
            }
        } else {
            // ターゲット文字列の長さを超えて入力された場合、不正解として��う
            playSound('incorrect');
            state.missCount++;
            state.combo = 0;
            elements.typingInput.value = state.currentInput; // 余分な入力を削除
        }
    }

    // 表示の更新
    let displayHtml = '';
    const isMeaningRecallMode = state.gameMode.includes('meaning_recall'); // 意味→語句タイピングモードを判定
    ...
    // 単語が完成したかチェック
    if (normalizeTypingString(state.currentInput).toLowerCase() === normalizeTypingString(targetText).toLowerCase()) {
        playSound('correct');
...
elements.testSubmitButton.addEventListener('click', () => {
    ...
    // 新しい判定ロジックを使用
    const isCorrect = checkSpellingTestAnswer(typedText, targetText);
